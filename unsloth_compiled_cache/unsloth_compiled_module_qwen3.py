...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.qwen3.modeling_qwen3 import (Callable, Optional, torch, nn, ACT2FN, Cache, use_kernel_func_from_hub, FlashAttentionKwargs, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, Unpack, TransformersKwargs, maybe_autocast, Qwen3Config)

@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def Qwen3RMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class Qwen3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return Qwen3RMSNorm_forward(self, hidden_states=hidden_states)
...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x):
        return Qwen3MLP_forward(self, x=x)
...
class Qwen3RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return Qwen3RotaryEmbedding_forward(self, x=x, position_ids=position_ids)
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return Qwen3Attention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)
